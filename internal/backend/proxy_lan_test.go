...
func TestE2E_LAN(t *testing.T) {
	...
	bd1 := NewBackend("", cs.Addr, NewLAN("198.51.100.1"))
	...
	bd2 := NewBackend("", cs.Addr, NewLAN("198.51.100.2"))
	...
	assert.Equal(t, "198.51.100.2", session1.gameRoom.Players["2"].IPAddress)
	...
	assert.Equal(t, "198.51.100.2", session2.gameRoom.Players["2"].IPAddress)
